...
| `orchestrator.interval_seconds` | Live | Manager resets its route timer from acceptance; scans remain serialized | Infallible generation publication follows durable commit | live interval wake test |
...
| `orchestrator.max_parallel` | Live | Resizable gate raises promptly; lowering preserves active jobs and blocks new claims until below bound | Infallible generation publication follows durable commit | live capacity test and race suite |
