...
- The configuration must be `.spynel/config.yaml` and match the current schema. Unused or obsolete keys are ignored on load without rewriting the file and are removed by the next canonical save; malformed current setting values fail validation with their source location.
