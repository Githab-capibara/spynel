# 01. Coding harness compatibility
...
Cline CLI ACP behavior was inspected on **2026-09-16**.
...
reviewed [provider-canary threat model and authorization plan](../security/01-provider-canary-threat-model.md).
...
| Cline CLI | `cline --acp`; stable ACP v1 JSON-RPC over stdio | **Not established.** The profile has no `--check` probe, so Spynel requires only a discoverable `cline` executable and then relies on the shared ACP v1 `initialize` negotiation. | Cline CLI `3.0.62` was executed on the Linux amd64 audit host from a revision-checkout launcher; `cline --acp` answered an unauthenticated `initialize` with `protocolVersion: 1`, agent name `cline`, and three sign-in methods. Shared synthetic ACP lifecycle/cancellation tests pass; no authenticated request was made. | Supported ACP alias with discovery and the shared ACP adapter. Sign-in and Hub state stay with the Cline CLI. See the [catalog](../../internal/harness/catalog.go), [ACP adapter](../../internal/harness/acp.go), and the official [Cline CLI documentation](https://docs.cline.bot/cline-cli/overview). |
| ACP v1 agents | Newline-delimited JSON-RPC 2.0 over a launched command's stdin/stdout | Protocol version **1**, negotiated by `initialize`; there is no numeric minimum agent release. | Synthetic initialize, session setup/resume, config-option, streaming, permission, cancellation, persistence, and shell-free argument tests pass. Alias commands were checked against official ACP registry manifests, and the `cline` and `opencode` alias executables completed an unauthenticated v1 `initialize` handshake on the audit host; no authenticated provider was run. | Supported shared adapter for catalog aliases and custom commands. Stable v1 stdio is supported; draft HTTP transport is deliberately unsupported. See the [ACP adapter](../../internal/harness/acp.go), [tests](../../internal/harness/acp_test.go), official [transport](https://agentclientprotocol.com/protocol/v1/transports), and [initialization](https://agentclientprotocol.com/protocol/v1/initialization) specifications. |
